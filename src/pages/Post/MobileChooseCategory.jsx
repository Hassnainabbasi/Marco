import { ArrowBackIos, ArrowBackOutlined, ArrowForward, ArrowForwardIos, ArrowRightAlt } from '@mui/icons-material'
...
export default function MobileChooseCategory({selectedMobileCategory}) {
  const [selectedCategory, setSelectedCategory] = useState(selectedMobileCategory);
  const [selectedSubcategory, setSelectedSubcategory] = useState(null);
  const [viewLevel, setViewLevel] = useState("subcategory");
  ...
  const subcategoryObj = categoryObj?.subcategories.find(
    (sub) => typeof sub === "object" && sub.name === selectedSubcategory
  );
 ...
 const navigate = useNavigate()
  return (
    <div className="">
     <div className='bg-gray-100 shadow-sm py-4'>
...
/>
        <h1 className="text-xl font-bold text-teal-950">
          {viewLevel === "subcategory" ? `${selectedCategory}` : `${selectedSubcategory}`}
        </h1>
      </div>
      ...
      <div className="flex">
            ...
            {categoryObj?.subcategories.map((sub, idx) => (
              <div
                ...
                className={`w-full px-7 h-12 flex items-center justify-between cursor-pointer 
                  ${selectedSubcategory === (sub.name || sub) ? "bg-teal-100" : "hover:bg-teal-100"}
                `}
                onClick={() => {
        ...
        {viewLevel === "subsubcategory" && (
          <div className="w-full">
            {subcategoryObj?.subcategories.map((nested, idx) => (
              <div
                key={idx}
                className="w-full px-7 h-12 flex items-center justify-between hover:bg-teal-100 cursor-pointer"
                onClick={() => {
                  navigate("/post/attributes");
                }}
              >
